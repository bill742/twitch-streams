    ...
    $.ajax
      ...
      success: (channels, textStatus, jqXHR) ->
        ...
        if name == undefined
          ...
          feedList.innerHTML += "<div class='stream-block " + channelStatus + "'><div class='stream-block-top' style='background-image: url(" + background + ");'><img src ='" + logo + "' alt='" + name + "' class='logo'></div><div class='stream-block-bottom'><a href='" + channelUrl + "' target='_blank' class='link'>View on Twitch</a><h3>" + name + "</h3><p class='channel-status'><span>Status:</span> " + channelStatus + "</p>" + current + "</div></div>"
